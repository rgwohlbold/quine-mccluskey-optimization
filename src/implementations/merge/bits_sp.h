...
static inline void merge_implicants_bits1(bitmap implicants, bitmap primes, size_t input_index, size_t output_index, int first_difference) {
...
static inline void merge_implicants_bits2(bitmap implicants, bitmap prime, size_t input_index, size_t output_index, int first_difference) {
...
static inline void merge_implicants_bits3(
    bitmap implicants,
    bitmap primes,
    size_t input_index,
    size_t output_index,
...
static inline void merge_implicants_bits4(
    bitmap implicants,
    bitmap primes,
    size_t input_index,
    size_t output_index,
...
static inline void merge_implicants_bits5(
    bitmap implicants,
    bitmap primes,
    size_t input_index,
    size_t output_index,
    ...
    // pointers into the two bitmaps (32 bits per word in implicants & primes)
    uint32_t *implicant_ptr = (uint32_t *)implicants.bits;
    uint32_t *primes_ptr     = (uint32_t *)primes.bits;
    // output_ptr writes 16-bit output words back into the implicants buffer
    uint16_t *output_ptr     = (uint16_t *)implicants.bits;

    // load the 32-bit word of implicants
    ...
    // write the prime‐mask (bits that never got merged)
...
static inline void merge_implicants_bits6(
    bitmap implicants,
    bitmap primes,
    size_t input_index,
    size_t output_index,
    ...
    // compute and store the prime‐mask
    uint64_t res_prime = impl & ~(merged1 | merged2 | merged3 | merged4 | merged5 | merged6);
    primes_ptr[input_index / 64] = res_prime;
...
static inline void merge_bits_sp(bitmap implicants, bitmap primes, size_t input_index, size_t output_index, int num_bits, int first_difference) {
    if (num_bits == 1) {
        merge_implicants_bits1(implicants, primes, input_index, output_index, first_difference);
        // LOG_DEBUG("Merging %d bits", num_bits);
        ...
        merge_implicants_bits6(implicants, primes, input_index, output_index, first_difference);
        // LOG_DEBUG("Merging %d bits", num_bits);
        // print_bitmap_sparse("primes", &primes);
        return;
    }
