...
static inline void merge_bits_sp_unroll(bitmap implicants, bitmap primes, size_t input_index, size_t output_index,
    ...
    for (int regist = 0; regist < num_registers; regist++) {
        ...
        LOG_DEBUG("Block: %d, idx1: %zu, oidx1: %zu, oidx2: %zu, first_difference: %d", regist, idx1, o_idx1, o_idx2,
                  first_difference);
        if (0 >= first_difference) {
    ...
    const int input_index_64 = input_index / 64;
    for (int i = 6; i < num_bits; i++) {
        int block_len = 1 << i;
        int block_len_64 = block_len / 64;
