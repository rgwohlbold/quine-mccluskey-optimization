...
static inline void init_tsc(void) {
    kperf_init();
}

static inline TIMESTAMP start_tsc(void) {
    struct performance_counters start;
    start = kperf_get_counters();
    return (TIMESTAMP) start.cycles;
}

static inline TIMESTAMP stop_tsc(TIMESTAMP start) {
    struct performance_counters end;
    end = kperf_get_counters();
    return (end.cycles -  (TIMESTAMP) start);
}
