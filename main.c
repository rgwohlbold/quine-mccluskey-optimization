...
implicant allocate_minterm_array(int num_bits) {
    implicant minterms = calloc((1 << num_bits) * num_bits, sizeof(ternary_value));
    if (minterms == NULL) {
...
bool *allocate_boolean_array(int num_bits) {
    bool *arr = calloc(1 << num_bits, sizeof(bool));
    if (arr == NULL) {
 ...
 */
implicant prime_implicants(int num_bits, int num_trues, int *trues, int num_dont_cares, int *dont_cares, int *num_prime_implicants) {
...
int main(int argc, char **argv) {
    ...
    implicant primes = prime_implicants(num_bits, sizeof(trues) / sizeof(trues[0]), trues, sizeof(dont_cares) / sizeof(dont_cares[0]), dont_cares, &num_prime_implicants);
